...
import { Message, FileAttachment, Conversation, User, QueuedItem, Language, CallRecord, MessageReaction, ReplyTo } from '@/types/chat';
import { mockSendMessage, mockUploadFile, mockReceiveReply, validateFile, getFileType } from '@/lib/mockApi';
...
interface ChatContextType {
  ...
  sendMessage: (content: string, attachments?: File[], options?: { isVanish?: boolean; viewOnce?: boolean }) => Promise<void>;
...
const mockCallHistory: CallRecord[] = [
  { id: 'call-1', type: 'video', user: mockUsers[0], timestamp: new Date(Date.now() - 3600000), duration: 125, status: 'completed', isOutgoing: true },
  { id: 'call-2', type: 'audio', user: mockUsers[1], timestamp: new Date(Date.now() - 7200000), duration: 0, status: 'missed', isOutgoing: false },
  { id: 'call-3', type: 'audio', user: mockUsers[2], timestamp: new Date(Date.now() - 86400000), duration: 45, status: 'completed', isOutgoing: false },
];

export function ChatProvider({ children }: { children: React.ReactNode }) {
  const [messages, setMessages] = useState<Message[]>([]);
  const [conversations, setConversations] = useState<Conversation[]>(initialConversations);
  const [activeConversation, setActiveConversationState] = useState<Conversation | null>(initialConversations[0]);
  ...
  const processMessageSend = async (message: Message) => {
    for (const attachment of message.attachments) {
      if (attachment.uploadStatus !== 'sent' && attachment.uploadStatus !== 'delivered' && attachment.uploadStatus !== 'read') {
        await processFileUpload(message.id, attachment);
      }
    }
    const result = await mockSendMessage(message);
    setMessages((prev) => prev.map((m) => m.id === message.id ? { ...m, status: result.status } : m));
    if (result.success) {
      setTimeout(() => {
        setMessages((prev) => prev.map((m) => m.id === message.id && m.status === 'sent' ? { ...m, status: 'delivered' } : m));
      }, 500);
      setTimeout(() => {
        setMessages((prev) => prev.map((m) => m.id === message.id && (m.status === 'sent' || m.status === 'delivered') ? { ...m, status: 'read' } : m));
      }, 1000 + Math.random() * 1000);
      if (typingCleanup.current) typingCleanup.current();
      typingCleanup.current = mockTypingIndicator(() => setIsTyping(true), () => setIsTyping(false));
  ...
  const processFileUpload = async (messageId: string, file: FileAttachment) => {
    return new Promise<void>((resolve) => {
      mockUploadFile(file, (progress) => {
        setMessages((prev) => prev.map((m) => m.id === messageId ? { ...m, attachments: m.attachments.map((a) => a.id === file.id ? { ...a, uploadProgress: progress } : a) } : m));
      }).then((result) => {
        setMessages((prev) => prev.map((m) => m.id === messageId ? { ...m, attachments: m.attachments.map((a) => a.id === file.id ? { ...a, uploadStatus: result.status, uploadProgress: result.success ? 100 : 0 } : a) } : m));
        resolve();
      });
    });
  };

  const sendMessage = useCallback(async (content: string, files?: File[], options?: { isVanish?: boolean; viewOnce?: boolean }) => {
  ...
  const retryMessage = useCallback(async (messageId: string) => {
    ...
    if (!message) return;
    setMessages((prev) => prev.map((m) => m.id === messageId ? { ...m, status: 'pending', attachments: m.attachments.map((a) => a.uploadStatus === 'failed' ? { ...a, uploadStatus: 'pending', uploadProgress: 0 } : a) } : m));
    if (!isOnline) {
      setQueue((prev) => [...prev, { 
  ...
  const retryFileUpload = useCallback(async (messageId: string, fileId: string) => {
    const message = messages.find((m) => m.id === messageId);
    const file = message?.attachments.find((a) => a.id === fileId);
    if (!message || !file) return;
    setMessages((prev) => prev.map((m) => m.id === messageId ? { ...m, attachments: m.attachments.map((a) => a.id === fileId ? { ...a, uploadStatus: 'pending', uploadProgress: 0 } : a) } : m));
    if (!isOnline) return;
    await processFileUpload(messageId, file);
  }, [messages, isOnline]);

  const editMessage = useCallback((messageId: string, newContent: string) => {
    setMessages((prev) => prev.map((m) => m.id === messageId ? { ...m, content: newContent, isEdited: true, editedAt: new Date() } : m));
  }, []);
  ...
  const addReaction = useCallback((messageId: string, emoji: string) => {
    const reaction: MessageReaction = { emoji, odontUserId: currentUser.id, userName: currentUser.name, timestamp: new Date() };
    setMessages((prev) => prev.map((m) => m.id === messageId ? { ...m, reactions: [...(m.reactions || []), reaction] } : m));
  }, []);

  const removeReaction = useCallback((messageId: string, emoji: string) => {
    setMessages((prev) => prev.map((m) => m.id === messageId ? { ...m, reactions: (m.reactions || []).filter((r) => !(r.emoji === emoji && r.odontUserId === currentUser.id)) } : m));
  }, []);
  ...
  const setTypingCallback = useCallback((typing: boolean) => {
    ...
    if (typing) {
      userTypingTimeout.current = setTimeout(() => {
        if (activeConversation?.isGroup) {
          ...
          setTypingUsers(groupUsers.sort(() => Math.random() - 0.5).slice(0, typingCount).map((u) => u.name));
        }
      }, 500);
    } else {
      setTypingUsers([]);
  ...
  const toggleVanishMode = useCallback((conversationId: string, enabled: boolean, timer = 60) => {
    setConversations((prev) => prev.map((c) => c.id === conversationId ? { ...c, isVanishMode: enabled, vanishTimer: timer } : c));
    if (activeConversation?.id === conversationId) {
      setActiveConversationState((prev) => prev ? { ...prev, isVanishMode: enabled, vanishTimer: timer } : prev);
    }
  }, [activeConversation]);
  ...
  const markViewOnceAsViewed = useCallback((messageId: string, attachmentId: string) => {
    setMessages((prev) => prev.map((m) => 
      m.id === messageId 
        ? { ...m, attachments: m.attachments.map((a) => a.id === attachmentId ? { ...a, isViewed: true } : a) } 
        : m
    ));
  }, []);
  ...
  const value: ChatContextType = {
    messages, conversations, activeConversation, isOnline, queue, language, currentUser,
    isTyping, typingUsers, showConversationList, callHistory, allUsers: mockUsers, searchQuery, replyingTo,
    isProcessingQueue, pinnedMessages,
    sendMessage, retryMessage, retryFileUpload, editMessage, deleteMessage, addReaction, removeReaction,
    setActiveConversation, toggleOnline, setLanguage, translate, setTyping: setTypingCallback,
    setShowConversationList, addCallToHistory, startNewChat, setSearchQuery, createGroup,
    toggleVanishMode, setReplyingTo, forwardMessage, markViewOnceAsViewed, pinMessage, unpinMessage,
  };
